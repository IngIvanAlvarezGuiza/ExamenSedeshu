...
function paginacion(){
    ...
    dataTable = $('#listaTipoNominas').dataTable( {
        "oLanguage":{
...
function seleccionMultiple(id){
    ...
    if(("#chk"+id) == '#chkTodo'){
        ...
        if($("#chk"+id).is(':checked')){
            ...
            var cont = 0;
            $('#listaTipoNominas tbody input[type=checkbox]').each(function(){
                cont = cont + 1;
            });
...
function ultimoTipoNomina(){

  var uri = '/tiponomina/obtenerUltimoID';
  var id = 0;
...
function modalTipoNominas(operacion,_id){
    ...
    if(operacion != "Agregar"){
      ...
      $.getJSON(uri,function(item){
            modal = '<div class="modal fade" id="modalTipoNomina" tabindex="-1" role="dialog" aria-labelledby="modalTipoNomina" aria-hidden="true">'+
                '<div class="modal-dialog" role="document">'+
                  '<div class="modal-content">'+
                     '<div class="modal-header" style="background-color: rgb(77,182,172);color: white">'+
                      '<h5 class="modal-title" style="margin: 0 auto;">'+operacion+' tipo de nómina <i class="fa fa-clock-o" aria-hidden="true"></i></h5>'+
                      ...
                      '<div class="row">'+
                         '<div class="form-group col-md-10 offset-1">'+
                              ...
                              if(operacion=="Consultar"){
                                  modal+='<input type="text" disabled class="form-control" value="'+item.tipnomina_desc+'" id="txtDescripcion" >';
                              }else{
                                  modal+='<input type="text" maxlength="250" class="form-control" value="'+item.tipnomina_desc+'" id="txtDescripcion" >';
                              }
...
function guardar(operacion){
    if(esCorrecto()){
        ...
        if(operacion != "Agregar"){
            ...
            if(rep){
                alerta('El tipo de nómina <strong>'+$('#txtDescripcion').val()+'</strong> ya existe.','info');
            }else{
                guardarInformacion(operacion);
            }
        }
...
function repetido(){
     var uri = '/tiponomina/'+$.trim($('#txtDescripcion').val());
     var bandera = '';
...
function guardarInformacion(operacion){
  ...
  if(operacion != "Agregar"){
    tipo = 'PUT';
    uri = '/tiponominas/'+parseInt($('#txtClave').val())+"";
  }else{
    tipo = 'POST';
    uri = '/tiponominas/';
  }
  ...
  $.ajax({
    ...
    data:{
      ...
      "tipnomina_cve":parseInt($('#txtClave').val()),
      "tipnomina_desc":$.trim($('#txtDescripcion').val())
    },
...
function exito(){
    ...
    setTimeout(function(){
        $('#modalTipoNomina .modal-body  #error').html('');
        cargarTabla();
        cerrarModal();
    },2000);
...
function alerta(mensaje,tipo){
  ...
  var alerta = 
    ...
    $('#modalTipoNomina .modal-body  #error').html('');
    $('#modalTipoNomina .modal-body  #error').append(alerta);
                        ...
                        '<td>'+
                           '<div class="form-check">'+
                              '<label class="custom-control custom-checkbox">'+
                                  '<input onclick=\'seleccionMultiple('+item.tipnomina_cve+')\'\ id="chk'+item.tipnomina_cve+'"  type="checkbox" name="chkSeleccionar" class="custom-control-input" value="'+parseInt(item.tipnomina_cve)+'">'+
                                  '<span class="custom-control-indicator"></span>'+
                              '</label>'+
                            '</div>'+
                        '</td>'+
                        '<td width="10px">'+item.tipnomina_cve+'</td>'+
                        '<td width="80%">'+item.tipnomina_desc+'</td>'+
                        '<td  width="13%">'+
                            '<button title="Editar" type="button" class="funct btn btn-outline-success" onclick=\'modalTipoNominas("Editar",'+parseInt(item.tipnomina_cve)+')\'\><i class="fa fa-pencil" aria-hidden="true"></i></button>'+
                            '<button title="Consultar" type="button" class="funct btn btn-outline-info" onclick=\'modalTipoNominas("Consultar",'+parseInt(item.tipnomina_cve)+')\'\><i class="fa fa-eye" aria-hidden="true"></i></button>'+
                        '</td> '+
                     '</tr>';
        });

         $('#listaTipoNominas tbody').append(tabla);
    }); 
...
function cerrarModal(){
  ...
  $("#modalTipoNomina").modal('hide').remove();
  $('.modal-backdrop.show').remove();
  $('.modal-open').removeClass();
  $('#modalTipoNomina').remove();
}
